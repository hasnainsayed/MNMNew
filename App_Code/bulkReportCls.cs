        ...
        try
        {
            ...
            if (btnType.Equals("VendorWise Stock"))
            {
                command.CommandText = "SELECT substring(c1name, CHARINDEX(':', c1name)+1, len(C1Name)-(CHARINDEX(':',c1name)-1)) AS 'VendorName', " +
                                    ...
                                    "FROM( " +
                                    "SELECT " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 30 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS pless30s, 0 AS pless30w, 0 AS a0to30, " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 30 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 60 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p31to60s, 0 AS p31to60w, 0 AS a31to60, " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 60 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 90 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p61to90s, 0 AS p61to90w, 0 AS a61to90, " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 90 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 120 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p91to120s, 0 AS p91to120w, 0 AS a91to120, " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 120 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 150 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p121to150s, 0 AS p121to150w, 0 AS a121to150, " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 150 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 180 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p151to180s, 0 AS p151to180w, 0 AS a151to180, " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 180 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 240 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p181to240s, 0 AS p181to240w, 0 AS a181to240, " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 240 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 300 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p241to300s, 0 AS p241to300w, 0 AS a241to300, " +
                                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 300 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 360 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p301to360s, 0 AS p301to360w, 0 AS a301to360, " +
                                    "(CASE WHEN((DATEDIFF(DAY, s.SystemDate, GETDATE())) > 360) Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS pmore360s, 0 AS pmore360w, 0 AS amore360, " +
                                    "((CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END )) AS totals, 0 AS totalw, 0 AS totallr, " +
                                    "c.c1name " +
                                    "FROM StockUpInward s " +
                                    "INNER JOIN ItemStyle i ON i.styleid = s.styleid INNER JOIN COLUMN1 c ON c.col1id = i.Col1 INNER  JOIN Vendor v ON v.svid=c.Col1ID " +
                                    "WHERE s.physicalid = 3 " +
                                    "GROUP BY s.SystemDate, c.c1name, v.istax,s.travelCost " +
                                    "UNION ALL " +
                                    "SELECT " +
                                    "0 as pless30s, (CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 30 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS pless30w, 0 AS a0to30, " +
                                    "0 AS p31to60s, (CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 30 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 60 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p31to60w, 0 AS a31to60, " +
                                    "0 AS p61to60s, (CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 60 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 90 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p61to90w, 0 AS a61to90, " +
                                    "0 AS p91to60s, (CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 90 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 120 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p91to120w, 0 AS a91to120, " +
                                    "0 AS p121to60s, (CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 120 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 150 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p121to150w, 0 AS a121to150, " +
                                    "0 AS p151to60s, (CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 150 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 180 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p151to180w, 0 AS a151to180, " +
                                    "0 AS p181to60s, (CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 180 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 240 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p181to240w, 0 AS a181to240, " +
                                    "0 AS p241to60s, (CASE WHEN(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 240 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 300 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p241to300w, 0 AS a241to300, " +
                                    "0 AS p301to60s, (CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 300 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 360 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p301to360w, 0 AS a301to360, " +
                                    "0 AS pmore360s, (CASE WHEN((DATEDIFF(DAY, s.SystemDate, GETDATE())) > 360) Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS pmore360w, 0 AS amore360, " +
                                    "0 AS totals, ((CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END )) AS totalw, 0 AS totallr, " +
                                    "c.c1name " +
                                    "FROM StockUpInward s " +
                                    "INNER JOIN ItemStyle i ON i.styleid = s.styleid INNER JOIN COLUMN1 c ON c.col1id = i.Col1 INNER  JOIN Vendor v ON v.svid=c.Col1ID " +
                                    "WHERE s.physicalid <> 3 " +
            ...
            else if (btnType.Equals("VendorWise Stock Shop"))
            {
                command.CommandText = "SELECT substring(c1name, CHARINDEX(':', c1name)+1, len(C1Name)-(CHARINDEX(':',c1name)-1)) AS 'Vendor', SUM(pless30) AS '0-30' , SUM(p31to60) AS '31-60', SUM(p61to90) AS '61-90', SUM(p91to120) AS '91-120', SUM(p121to150) AS '121-150', SUM(p151to180) AS '151-180', SUM(p181to240) AS '181-240', SUM(p241to300) AS '241-300', SUM(p301to360) AS '301-360', SUM(pmore360) AS '360+', SUM(total) AS 'Grand Total' " +
                    "FROM( " +
                    "SELECT " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 30 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS pless30, " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 30 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 60 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p31to60, " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 60 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 90 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p61to90, " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 90 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 120 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p91to120, " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 120 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 150 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p121to150, " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 150 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 180 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p151to180, " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 180 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 240 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p181to240, " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 240 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 300 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p241to300, " +
                    "(CASE When(DATEDIFF(DAY, s.SystemDate, GETDATE())) > 300 AND(DATEDIFF(DAY, s.SystemDate, GETDATE())) <= 360 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS p301to360, " +
                    "(CASE WHEN((DATEDIFF(DAY, s.SystemDate, GETDATE())) > 360) Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END ) Else 0 END) AS pmore360, " +
                    "((CASE WHEN isnull(v.istax,0)=1 THEN sum(s.purchaseRate)+((SELECT tax FROM tax)/100* sum(s.purchaseRate))+sum(ISNULL(s.travelCost,0)) ELSE sum(s.purchaseRate)+sum(isnull(s.travelCost,0)) END )) AS total, " +
                    "c.c1name, s.purchaseRate, s.SystemDate " +
                    "FROM StockUpInward s " +
                    "INNER JOIN ItemStyle i ON i.styleid = s.styleid INNER JOIN COLUMN1 c ON c.col1id = i.Col1 INNER  JOIN Vendor v ON v.svid=c.Col1ID WHERE s.physicalId = 3 GROUP BY s.SystemDate, c.c1name, s.purchaseRate,v.istax,s.travelCost) a GROUP BY c1name ORDER BY c1name";
            }
            ...
            else if (btnType.Equals("VendorWise Stock Warehouse"))
            {
                command.CommandText = "SELECT substring(c1name, CHARINDEX(':', c1name)+1, len(C1Name)-(CHARINDEX(':',c1name)-1)) AS 'Vendor', SUM(pless30) AS '0-30' , SUM(p31to60) AS '31-60', SUM(p61to90) AS '61-90', SUM(p91to120) AS '91-120', SUM(p121to150) AS '121-150', SUM(p151to180) AS '151-180', SUM(p181to240) AS '181-240', SUM(p241to300) AS '241-300', SUM(p301to360) AS '301-360', SUM(pmore360) AS '360+', SUM(total) AS 'Grand Total' " +
                     ...
                     "c.c1name, s.purchaseRate, s.SystemDate " +
                     "FROM StockUpInward s " +
                     "INNER JOIN ItemStyle i ON i.styleid = s.styleid INNER JOIN COLUMN1 c ON c.col1id = i.Col1 INNER  JOIN Vendor v ON v.svid=c.Col1ID WHERE s.physicalId = 1 GROUP BY s.SystemDate, c.c1name, s.purchaseRate,v.istax,s.travelCost) a GROUP BY c1name ORDER BY c1name";
            }
            ...
            else if (btnType.Equals("Sales With Margin"))
            {
                command.CommandText = "SELECT substring(c1name, CHARINDEX(':', c1name)+1, len(C1Name)-(CHARINDEX(':',c1name)-1)) AS 'Vendor', " +
                      ...
                      "FROM( " +
                      "SELECT " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 30 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS pless30, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 30 Then s.sellingprice Else 0 END) AS sless30, " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 30 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 60 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS p31to60, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 30 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 60 THEN s.sellingprice Else 0 END) AS s31to60, " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 60 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 90 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS p61to90, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 60 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 90 Then s.sellingprice Else 0 END) AS s61to90, " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 90 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 120 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS p91to120, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 90 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 120 Then s.sellingprice Else 0 END) AS s91to120, " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 120 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 150 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS p121to150, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 120 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 150 Then s.sellingprice Else 0 END) AS s121to150, " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 150 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 180 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS p151to180, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 150 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 180 Then s.sellingprice Else 0 END) AS s151to180, " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 180 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 240 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS p181to240, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 180 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 240 Then s.sellingprice Else 0 END) AS s181to240, " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 240 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 300 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS p241to300, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 240 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 300 Then s.sellingprice Else 0 END) AS s241to300, " +
                      "(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 300 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 360 Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS p301to360, SUM(CASE When(DATEDIFF(DAY, a.SystemDate, GETDATE())) > 300 AND(DATEDIFF(DAY, a.SystemDate, GETDATE())) <= 360 Then s.sellingprice Else 0 END) AS s301to360, " +
                      "(CASE WHEN((DATEDIFF(DAY, a.SystemDate, GETDATE())) > 360) Then (CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END ) Else 0 END) AS pmore360, SUM(CASE WHEN((DATEDIFF(DAY, a.SystemDate, GETDATE())) > 360) Then s.sellingprice Else 0 END) AS smore360, " +
                      "((CASE WHEN isnull(v.istax,0)=1 THEN sum(a.purchaseRate)+((SELECT tax FROM tax)/100* sum(a.purchaseRate))+sum(ISNULL(a.travelCost,0)) ELSE sum(a.purchaseRate)+sum(isnull(a.travelCost,0)) END )) AS totalpurchase, SUM(s.sellingprice) AS totalsales, " +
                      "c.c1name, a.purchaseRate, a.SystemDate " +
                      "FROM ArchiveStockUpInward a " +
                      "INNER JOIN ItemStyle i ON i.styleid = a.styleid INNER JOIN COLUMN1 c ON c.col1id = i.Col1 inner join salesrecord s ON s.archiveid = a.ArchiveStockupId INNER  JOIN Vendor v ON v.svid=c.Col1ID WHERE s.returntimestamp IS NULL AND convert(date, s.salesDateTime) BETWEEN convert(date, @frmDate) AND convert(date, @toDate) GROUP BY a.SystemDate, c.c1name, a.purchaseRate,v.istax,a.travelCost) b GROUP BY c1name ORDER BY c1name ";
            }
